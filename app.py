...
def get_checklist_items():
    fallback = {item["item_key"]: dict(item) for item in ROUTINE_FALLBACK_ITEMS}
    try:
    ...
    return sorted(merged.values(), key=lambda x: (x.get("sort_order") or 999, x.get("name") or ""))
...
FOOD_ONLY_KEYS = {"suco_verde"}
NON_ROUTINE_TERMS = ["suco", "café da manhã", "almoco", "almoço", "jantar", "ceia", "agua", "água", "bebida"]
...
def get_month_targets_preview(end_date):
    rows = get_monthly_target_rows(end_date)
    if not rows:
        return []
    if len(rows) == 1:
        return rows
...
def compute_daily_score(dt_iso, goals, meals, checklist, sleep, workout=None, hydration=None):
    workout = workout or {}
    ...
    kcal = totals["kcal"]
    prot = totals["prot"]
    alco_keys = {"cerveja_garrafa","caipirinha","gin_tonica","whisky_dose","vodka_mixer_zero"}
    had_alcohol = any((m.get("food_key") or "") in alco_keys for m in meals)
    if kcal <= 0:
        food_score = 0
    else:
        food_energy = 15 if kcal <= kcal_goal * 1.05 else 12 if kcal <= kcal_goal * 1.15 else 8 if kcal <= kcal_goal * 1.30 else 3
        food_structure = 10 if meal_count >= 4 else 8 if meal_count >= 3 else 5 if meal_count >= 2 else 2
        context = 5
        if had_alcohol:
            context -= 2
        if kcal > kcal_goal * 1.30:
            context -= 2
        food_score = food_energy + food_structure + max(0, context)
    if prot >= protein_goal:
        protein_score = 20
    elif prot >= protein_goal * 0.9:
        protein_score = 17
    elif prot >= protein_goal * 0.75:
        protein_score = 12
    elif prot > 0:
        protein_score = 6
    else:
    ...
    training_score = 20 if trained and duration >= 30 else 14 if trained else 0
    total_hours = float(sleep.get("total_hours") or 0) if sleep else 0
    used_cpap = bool(sleep.get("used_cpap")) if sleep else False
    sleep_score = 15 if total_hours >= 7 and used_cpap else 11 if total_hours >= 6 else 6 if total_hours > 0 else 0
    routine_score = round((routine_done / routine_total) * 10) if routine_total else 0
    water_score = 5 if water_ml >= water_goal else 4 if water_ml >= water_goal * 0.75 else 2 if water_ml >= water_goal * 0.5 else 1 if water_ml > 0 else 0
    total_score = food_score + protein_score + training_score + sleep_score + routine_score + water_score
    if training_score == 0 and protein_score <= 6:
        total_score = min(total_score, 59)
    band = "dia forte" if total_score >= 85 else "dia bom" if total_score >= 70 else "dia parcial" if total_score >= 55 else "dia fraco"
    return {"total": int(total_score), "band": band, "breakdown": {"alimentacao": int(food_score), "proteina": int(protein_score), "treino": int(training_score), "sono_cpap": int(sleep_score), "rotina": int(routine_score), "agua": int(water_score)}}


def save_daily_closure(dt_iso, payload):
    return safe_execute(lambda: db.table("daily_closure").upsert({"date": dt_iso, **payload}, on_conflict="date").execute())


def render_score_breakdown(score_data):
    bd = score_data["breakdown"]
    items = [
        ("Alimentação", bd["alimentacao"], 30),
        ("Proteína", bd["proteina"], 20),
        ("Treino", bd["treino"], 20),
        ("Sono + CPAP", bd["sono_cpap"], 15),
        ("Rotina", bd["rotina"], 10),
        ("Água", bd["agua"], 5),
    ]
    html = '<div class="card"><div class="meal-name">Score do dia</div>'
    html += f'<div class="big" style="margin-top:8px">{score_data["total"]}</div>'
    html += f'<div class="muted">{score_data["band"]}</div>'
    for label, val, total in items:
        pct = 0 if total == 0 else int((val / total) * 100)
        html += f'<div style="margin-top:10px"><div class="meal-detail">{label} · {val}/{total}</div><div class="progress-bar"><div class="progress-fill" style="width:{pct}%"></div></div></div>'
    html += '</div>'
...
def render_strategy_current(goals, current_weight=None):
    ...
    long_goal = PROJECT_PROFILE["goal_weight_final"]
    interim_goal = PROJECT_PROFILE["goal_weight_intermediate"]
    kcal = t["kcal"]
    protein = t["protein"]
    carb = t["carb"]
    fat = t["fat"]
...
def render_graph(end_date, goal_weight):
    ...
    st.line_chart(df.set_index("date")[["peso_real", "curva_ideal", "curva_min"]], height=260, use_container_width=True)
    st.markdown('<div class="legend-wrap"><div class="legend-badge">Peso real</div><div class="legend-badge">Curva ideal</div><div class="legend-badge">Curva mínima aceitável</div></div>', unsafe_allow_html=True)
    previews = get_month_targets_preview(end_date)
    if previews:
        cols = st.columns(len(previews))
        for col, row in zip(cols, previews):
            with col:
                st.markdown(f'<div class="meal-card"><div class="meal-detail">Meta {row["month_label"]}</div><div class="meal-name">{row["target_weight"]:.1f} kg</div></div>', unsafe_allow_html=True)
    ...
    if not current_real.empty:
        ...
        st.caption(f'Peso atual {last_real:.1f} kg · curva ideal hoje {curve_info["projected_ideal"]:.1f} kg · status: {curve_info["label"]}.')
    else:
        st.caption('Registre o peso para comparar sua linha real com o ritmo esperado.')
...
def page_hoje():
    ...
    score_data = compute_daily_score(target, goals, meals, checklist, sleep, workout=workout, hydration=hydration)

    st.markdown('<div class="section-title">Resumo do dia</div>', unsafe_allow_html=True)
    render_day_summary_band(current_weight, PROJECT_PROFILE["goal_weight_intermediate"], meals, checklist, sleep, workout)

    c1, c2 = st.columns([1.45, 1])
    with c1:
        st.markdown('<div class="section-title">Ritmo do peso</div>', unsafe_allow_html=True)
    ...
    with c1:
        pv = float(current_weight or PROJECT_PROFILE["start_weight"])
    ...
    st.markdown('<div class="section-title">Treino e água</div>', unsafe_allow_html=True)
    a, b = st.columns([1, 1])
    with a:
        trained = bool(checklist.get("treino", {}).get("done")) or bool(workout.get("workout_type"))
        if trained:
            ...
            st.markdown('<div class="meal-card"><div class="meal-name">Sem treino marcado</div><div class="meal-detail">Se musculação não entrar hoje, escolha um mínimo viável agora.</div></div>', unsafe_allow_html=True)
    with b:
    ...
    if st.button('Finalizar dia e gerar score', use_container_width=True, key=f'close_day_{target}'):
        analysis_html = build_day_review(target, goals, meals, checklist, sleep, current_weight, workout=workout, hydration=hydration)
        payload = {
            'score_total': score_data['total'],
            'score_band': score_data['band'],
            'score_breakdown_json': json.dumps(score_data['breakdown'], ensure_ascii=False),
            'analysis_html': analysis_html,
            'closed_at': datetime.now(APP_TZ).isoformat(),
        }
        ok, msg = save_daily_closure(target, payload)
        if ok:
            st.success(f'Dia fechado com score {score_data["total"]}/100.')
        else:
            st.warning('Tabela daily_closure ainda não existe. Rode o SQL antes de usar o fechamento.')
